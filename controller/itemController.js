...
async function getItemAmount() {
    try {
        const result = await modelItem.count({item_status: 3});
        console.log(result);
...
async function getItemStatusScheme() {
    try {
        const status = await modelItemStatus.find({}, {_id: 0, __v: 0});
        const result = {};
 ...
 * @returns
 */
async function getItemUsers({department_name = null, user_name = null}) {
    try {
        const query = {};
        ...
        const users = await modelUsers.aggregate([
            {
                $match: query,
            },
            {
 ...
 * @returns
 */
async function getUserNameById({user_id = null}) {
    try {
        const userName = await modelUsers.findOne(
            {_id: user_id},
            {_id: 0, user_name: 1}
        );
 ...
 * @returns
 */
async function getUserRank({user_id = null}) {
    try {
        const user = await modelUsers.findOne({_id: user_id}, {role_name: 1});
        const permissions = await modelRoleMapPermission.find(
        ...
        const status = await modelStatusMapPermissions.find({
            permission_identifier: {$in: identifier},
        });
        const statusMap = {};
        for (let i = 0; i < status.length; i++) {
        ...
        let manageStatus = await modelStatusType.findOne(
            {status_type: "manage_status"},
            {status_ids: 1}
        );
        let auditStatus = await modelStatusType.findOne(
            {status_type: "audit_status"},
            {status_ids: 1}
        );
...
async function getRuleTree() {
    try {
        const rules = await modelRule.find(
            {rule_name: {$ne: "null"}},
            {__v: 0, children: 0}
        );
        const res = {};
...
async function getRegionTree() {
    try {
        const regions = await modelRegion.find({}, {__v: 0, children: 0});
        const res = {};
...
async function getItems(
    ...
    try {
        ...
        create_end_time =
            create_end_time !== null ? create_end_time : 9999999999999;
        query.create_time = {$gte: create_start_time, $lte: create_end_time};
        release_start_time =
            release_start_time !== null ? release_start_time : 0;
        release_end_time =
            release_end_time !== null ? release_end_time : 9999999999999;
        query.release_time = {
            $gte: release_start_time,
            $lte: release_end_time,
        };
        if (
        ...
        if (page_size !== null && page_num !== null) {
            ...
            //计算规则路径和区划路径
            const ruleDic = itemService.getRuleDic()
            const regionDic = itemService.getRegionDic()
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                let node = ruleDic.has(data[i].rule_id)
                    ? ruleDic.get(data[i].rule_id)
                    : null

                while (node !== null) {
                    rulePath = node.rule_name + "/" + rulePath;
                    node = ruleDic.has(node.parentId)
                ...
                // 生成区划路径
                let regionPath = "";
                let node1 = regionDic[data[i].region_id]
                    ? regionDic[data[i].region_id]
                    : null;
                while (node1 !== null) {
                    regionPath = node1.region_name + "/" + regionPath;
                    node1 = regionDic[node1.parentId]
                        ? regionDic[node1.parentId]
                        : null;
                }
            ...
            dict.data = data;
            dict.total =
                items[0].count.length > 0 ? items[0].count[0].total : 0;
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let node = ruleDic.has(items[i].rule_id)
                ? ruleDic.get(items[i].rule_id)
                : null
            ...
            // 生成区划路径
            let regionPath = "";
            let node1 = regionDic[items[i].region_id]
                ? regionDic[items[i].region_id]
                : null;
            while (node1 !== null) {
                regionPath = node1.region_name + "/" + regionPath;
                node1 = regionDic[node1.parentId]
...
async function getMaxRuleId() {
    console.log("初始化maxRuleId")
    let res = 0
    let rules = await modelRule.find({}, {_id: 0, __v: 0});
    for (let i = 0; i < rules.length; i++) {
        res = Math.max(res, rules[i].rule_id)
    }
    return res + 1
}
...
async function initMaxRuleId() {
    try {
        ...
        console.log("maxRuleId", maxRuleId)
    } catch (e) {
        console.log('初始化 maxRuleId 失败')
        // 尝试重新初始化
        setTimeout(initMaxRuleId, 1000)
    }
}
...
async function createRules({user_id = null, rules = null}) {
    ...
    try {
        ...
        // var stakes = await modelRule.find(
        //     { rule_name: "null" },
        //     { _id: 0, __v: 0 }
        // );
        ...
        // try {
        //     await modelRule.updateOne(
        //         { rule_id: stake.rule_id },
        ...
        // parentChildrenMapping 的 key 是 rules[i].temp_id，value 是规则节点 id: rules[i].rule_id
        const tempIdToRuleIdMapping = new Map()

        //给传入的规则节点创建rule_id
        for (let i = 0; i < rules.length; i++) {
            rules[i].rule_id = maxRuleId.toString();
            tempIdToRuleIdMapping.set(rules[i].temp_id, rules[i].rule_id)
            maxRuleId = maxRuleId + 1;
        }

        //修改 parentId 使它们指向 rule_id 而不是 temp_id
        for (let i = 0; i < rules.length; i++) {
            if (tempIdToRuleIdMapping.has(rules[i].parentId)) {
                rules[i].parentId = tempIdToRuleIdMapping.get(rules[i].parentId)
            }
        }

        //创建规则
        const rulesDoc = rules.map(rule => {
            return {
        ...
        const bulkOps = rules.map(rule => {
            return {
                updateOne: {
                    ...
                    update: {$push: {children: rule.rule_id}},
                },
            }
        })
        ...
        for (let i = 0; i < rules.length; i++) {
            res[rules[i].temp_id] = {
                rule_id: tempIdToRuleIdMapping.get(rules[i].temp_id)
            }
        }
...
//         //找出最大id并创建一个新的桩
//         var maxRuleId = 0;
//         var rules = await modelRule.find(
//             { rule_name: { $ne: "null" } },
//             { _id: 0, __v: 0 }
//         );
//         for (let i = 0; i < rules.length; i++) {
...
//     } catch (err) {
//         throw new Error(
//             "联系管理员检查数据库的rule表，确保rule_name为null的桩存在且rule_id是最大值"
//         );
//     }
// }
 ...
 * @returns
 */
async function deleteRules({rules = null}) {
    try {
        if (rules === null) {
        ...
        for (let i = 0; i < rules.length; i++) {
            //判断rule_id的合法性
            let rule = await modelRule.findOne(
                {rule_id: rules[i]},
                {_id: 0, __v: 0}
            );
        ...
        var items = await modelItem
            .find({rule_id: {$in: rules}}, {__v: 0})
            .count();
        if (items > 0) {
            return new SuccessModel({
                msg: "删除规则失败，有与其绑定的事项还未处理",
                data: {code: 999},
            });
        }
        //批量删除
 ...
 * @returns
 */
async function updateRules({rules = null}) {
    try {
        if (rules === null) {
            throw new Error("请求体中需要一个rules属性，且该属性是一个数组");
        }
 ...
 * @returns
 */
async function getRulePaths({rule_id = null}) {
    try {
        if (rule_id === null) {
        ...
        for (let i = 0; i < rule_id.length; i++) {
            let rulePath = [];
            let node = ruleDic.has(rule_id[i])
                ? ruleDic.get(rule_id[i]) : null

            while (node !== null) {
                rulePath.unshift(node)
                node = ruleDic.has(node.parentId) ?
                    ruleDic.get(node.parentId) : null;
            }
 ...
 * @returns
 */
async function getItemGuide({task_code = null}) {
    try {
        if (task_code === null) {
...
async function createItemGuide({
    ...
    try {
        ...
        if (legal_period_type !== null)
            newData.legal_period_type = legal_period_type;
        if (promised_period !== null) newData.promised_period = promised_period;
        if (promised_period_type !== null)
            newData.promised_period_type = promised_period_type;
        ...
        if (mobile_applt_website !== null)
            newData.mobile_applt_website = mobile_applt_website;
        if (submit_documents !== null)
            newData.submit_documents = submit_documents;
        if (zxpt !== null) newData.zxpt = zxpt;
        if (zzzd !== null) newData.zzzd = zzzd;
        if (service_agent_name !== null)
            newData.service_agent_name = service_agent_name;
        if (service_agent_code !== null)
            newData.service_agent_code = service_agent_code;
 ...
 * @returns
 */
async function deleteItemGuides({task_code = null}) {
    try {
        if (task_code === null) {
        ...
        //批量删除
        var result = await modelTask.deleteMany({
            task_code: {$in: task_code},
        });
...
async function updateItemGuide({
    ...
    try {
        ...
        if (new_task_code !== null) {
            ...
            if (task === true) {
                throw new Error(
                    "存在相同的事项指南编码，不能重复: " + new_task_code
                );
            }
        ...
        // }
        if (zzzd !== null) newData.zzzd = zzzd;
        const result = await modelTask.updateOne(
            {task_code: task_code},
            newData
        );
 ...
 * @returns
 */
async function getRegionPaths({region_id = null}) {
    try {
        if (region_id === null) {
            throw new Error(
                "请求体中需要一个region_id属性，且该属性是一个数组"
            );
        }
        ...
        for (let i = 0; i < region_id.length; i++) {
            ...
            while (node !== null) {
                ...
                node = regionDic[node.parentId]
                    ? regionDic[node.parentId]
                    : null;
            }
...
async function getRegions({
    ...
    try {
        ...
        if (parentCode !== null) {
            let codes = await modelRegion.find(
                {region_code: {$in: parentCode}},
                {__v: 0}
            );
            let parentid = [];
        ...
        if (page_size !== null && page_num !== null) {
            ...
            for (let i = 0; i < data.length; i++) {
                let regionPath = "";
                let node = regionDic[data[i]._id]
                    ? regionDic[data[i]._id]
                    : null;
                while (node !== null) {
            ...
            var dict = {};
            dict.data = data;
            dict.total = regions[0].count.length
                ? regions[0].count[0].total
                : 0;
            dict.page_size = page_size;
            dict.page_num = page_num;
            return new SuccessModel({msg: "查询成功", data: dict});
        ...
        for (let i = 0; i < regions.length; i++) {
            let regionPath = "";
            let node = regionDic[regions[i]._id]
                ? regionDic[regions[i]._id]
                : null;
            while (node !== null) {
                regionPath = node.region_name + "/" + regionPath;
                node = regionDic[node.parentId]
...
async function createItems({user_id = null, items = null}) {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let region = await modelRegion.findOne({
 ...
 * @returns
 */
async function updateItems({items = null}) {
    try {
        if (items === null) {
...
async function getChildRegionsByRuleAndRegion({
    ...
    try {
        ...
        const region = await modelRegion.findOne(
            {region_code: region_code},
            {__v: 0}
        );
        ...
        //先判断该区划本身是否有匹配的事项
        const doesExist = await doesRegionItemExist(
            rule_id,
            [region._id],
            service_object
        );
...
async function doesChildRegionItemExist(ruleId, regionIds, serviceObject) {
    return (
        (await modelItem.exists({
            rule_id: ruleId,
            region_id: {$in: regionIds},
        })) &&
        (await modelTask.exists({
            service_object_type: {
                $regex: serviceObjectTypeMapping(serviceObject),
            },
        }))
    );
}
...
async function getRules({
    ...
    try {
        ...
        //分页返回查询结果
        if (page_size !== null && page_num !== null) {
            const res = await modelRule.aggregate([
                {
                    $match: query,
                },
                {
        ...
        for (let i = 0; i < res.length; i++) {
            let rulePath = "";
            let node = ruleDic.has(res[i].rule_id)
                ? ruleDic.get(res[i].rule_id) : null
            ...
            const _res = await modelItem.aggregate([
                {
                    $match: {rule_id: res[i].rule_id},
                },
            ]);

            res[i].isLeaf = res[i].children.length === 0;
...
async function createRegion({
    ...
    try {
        ...
        if (
            ...
            throw new Error(
                "创建区划的时候必须包括全部字段的信息，包括region_code、region_name、region_level和parentCode"
            );
        }
        //检查user_id
        var user = await modelUsers.findOne({_id: user_id}, {__v: 0});
        ...
        //更新父区划的children数组
        await modelRegion.updateOne(
            {_id: parentId},
            {$push: {children: result._id.toString()}}
        );
        //更新内存中的区划树
 ...
 * @returns
 */
async function deleteRegions({regions = null}) {
    try {
        if (regions === null) {
        ...
        //检查是否有事项指南与区划绑定
        var items = await modelItem
            .find({region_id: {$in: regions}}, {__v: 0})
            .count();
        if (items > 0) {
            return new SuccessModel({
                msg: "删除区划失败，有与其绑定的事项还未处理",
                data: {code: 999},
            });
        }
        //批量删除
 ...
 * @returns
 */
async function updateRegions({regions = null}) {
    try {
        if (regions === null) {
            throw new Error("请求体中需要一个regions属性，且该属性是一个数组");
        }
 ...
 * @returns
 */
async function changeItemStatus({user_id = null, items = null}) {
    try {
        if (user_id === null || items === null) {
        ...
        for (let i = 0, len = items.length; i < len; i++) {
            ...
            if (item_id === null || next_status === null) {
                throw new Error(
                    "调用changeItemStatus必须有item_id和next_status字段"
                );
            }
            //查询事项现在的状态
            ...
            for (let j = 0; j < itemStatus.length; j++) {
                ...
                if (status.id !== item.item_status) {
                    if (j === itemStatus.length - 1)
                        throw new Error(
                            "itemStatus表和数据库中已有的事项状态对不上"
                        );
                    continue;
                }
                //判断能否变为next_status
...
async function addAuditAdvise({
    ...
    try {
        ...
        var result = await modelItem.updateOne(
            {_id: item_id},
            {audit_advises: advises}
        );
 ...
 * @returns
 */
async function getItemGuideAndAuditAdvises({item_id = null}) {
    try {
        if (item_id === null) {
            throw new Error("需要item_id");
        }
        var item = await modelItem.findOne(
            {_id: item_id},
            {task_code: 1, audit_advises: 1}
        );
        var itemGuide = await modelTask.findOne(
    ...
    try {
        ...
        for (let i = 0, len = keys.length; i < len; i++) {
            Array.prototype.push.apply(
                array[0].guides,
                result[keys[i]].inRemoteNinLocal
            );
            Array.prototype.push.apply(
                array[1].guides,
                result[keys[i]].inLocalNinRemote
            );
            Array.prototype.push.apply(
                array[2].guides,
                result[keys[i]].differences
            );
            Array.prototype.push.apply(
                array[0].handle,
                result[keys[i]].handle_inRemoteNinLocal
            );
            Array.prototype.push.apply(
                array[1].handle,
                result[keys[i]].handle_inLocalNinRemote
            );
            Array.prototype.push.apply(
                array[2].handle,
                result[keys[i]].handle_differences
            );
            Array.prototype.push.apply(
                array[1].item_name,
                result[keys[i]].inLocalNinRemoteGuideNames
            );
            Array.prototype.push.apply(
                array[2].item_name,
                result[keys[i]].differencesGuideNames
            );
        }
            ...
            if (change_index != -1) {
                ...
                for (let j = change_index; j < arr[i].handle.length - 1; j++) {
                    ...
                    if (old_handle == 1 && arr[i].handle[j] == 0) {
                        ...
                        break;
                    } else if (j + 1 == arr[i].handle.length - 1) {
                        //所有事项都未处理，则把该事项放到最后
